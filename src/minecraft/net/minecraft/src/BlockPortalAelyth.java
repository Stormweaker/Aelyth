...
public class BlockPortalAelyth extends BlockBreakable
{

    public BlockPortalAelyth(int i, int j)
    {
    ...
    public boolean tryToCreatePortal(World world, int i, int j, int k)
    {
        ...
        for(int l1 = -1; l1 <= 2; l1++)
        {
            for(int j1 = -1; j1 <= 3; j1++)
            {
                ...
                if(l1 != -1 && l1 != 2 || j1 != -1 && j1 != 3)
                {
                    ...
                    if(j2 != 0 && j2 != mod_Aelyth.AelythPortal.blockID)
                    {
                        return false;
                    }
                }
        ...
        for(int i2 = 0; i2 < 2; i2++)
        {
            ...
            {
                world.setBlockWithNotify(i + l * i2, j + k1, k + i1 * i2, mod_Aelyth.AelythPortal.blockID);
            }
    ...
    public void onEntityCollidedWithBlock(World world, int i, int j, int k, Entity entity)
    {
        if(entity.ridingEntity == null && entity.riddenByEntity == null)
        {
            if(entity instanceof EntityPlayerSP)
            {
                ...
                entityplayersp.dimension = getDimNumber();
            }
            entity.setInPortal();
        }
    }
